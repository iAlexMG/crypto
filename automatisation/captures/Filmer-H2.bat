@echo off
rem Double-clic = lance H2 d'un seul geste, pour filmer :
...
setlocal
start "Runner H2 (execute)" cmd /k "cd /d %~dp0.. && python runner_sma.py --strategie h2 --go"
rem laisse le runner creer le journal du jour avant d'attacher le suiveur
timeout /t 3 /nobreak >nul
start "Journal H2 (lecture seule)" powershell -NoExit -ExecutionPolicy Bypass -File "%~dp0suivre-journal.ps1" H2 -Neuf
endlocal
